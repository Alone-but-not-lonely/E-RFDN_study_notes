...
# Flickr2K 数据集定义 → 训练集
# =======================================
- 文件作用: 定义 Flickr2K 类，专用于 Flickr2K 数据集的加载。
...
class Flickr2K(srdata.SRData):
    ...
    # ---------------------------------------------------------------------
    def _scan(self):
        names_hr, names_lr = super(Flickr2K, self)._scan() # 调用父类的 _scan() 方法, 获取所有 HR/LR 图像文件名
    ...
    ''' 设置文件系统路径 '''
    # ---------------------------------------------------------------------
    # 作用: 重写 _set_filesystem() 方法, 设置 Flickr2K 数据集的文件系统路径。
    # 参数: 
    #   * dir_data: 数据集根目录路径。
    # ---------------------------------------------------------------------
    def _set_filesystem(self, dir_data):
        super(Flickr2K, self)._set_filesystem(dir_data) # 调用父类的 _set_filesystem() 方法, 设置基础路径
        self.dir_hr = os.path.join(self.apath, 'Flickr2K_HR') # 高分辨率图像路径
        self.dir_lr = os.path.join(self.apath, 'Flickr2K_LR_bicubic') # 低分辨率图像路径
