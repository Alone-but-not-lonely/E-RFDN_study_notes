...
# 兼容旧的训练框架 【修改部分】
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
* 作用：兼容旧的训练框架，允许在旧的代码基础上运行新的模型。这个函数是关键！它是 __init__.py 调用该模型的入口
# =========================================================================
'''
...
class RFDN(nn.Module):
    ...
    def __init__(self, args, in_nc=3, nf=50, num_modules=4, out_nc=3, upscale=4):
        ...
        # nf = args.n_feats  # 使用框架的 n_feats 参数，或者保持默认50
        upscale = args.scale[0] # EDSR框架的 scale 是一个列表 —— 【修改部分】
        ...
        self.upsampler = upsample_block(nf, out_nc, upscale_factor=upscale) # 使用 upscale 变量
